...
### 阶段3A · 快手生产发布验收（2026-08-26）

- 完成：原正式Sites兼容迁移与两条真实样本入库，未创建新站点、未进入3B。
- 备份：迁移前重新备份生产34表、35,228行，manifest及所有分块校验通过。
- 修复：旧内容查询在单平台范围内过滤作品和汇总卡，防止快手数据混入抖音响应；不改变抖音采集和评分逻辑。
- 验证：生产新增2作品、2快照、236趋势点、70来源、3评论、1粉丝快照、2评价；两次重复入口均返回already_processed，数据丢失/污染/重复为0；完整回归70/70。
- 说明：原热点服务在备份期间更新一条既有缺文件失败日志，时间早于迁移，单独记录并保留，未修改其流程。
- 详见 [阶段3A生产发布核验](docs/kuaishou-stage3a-production-release.md)。

### 阶段3A · 快手数据适配层 V1.0（原本地验收记录）
