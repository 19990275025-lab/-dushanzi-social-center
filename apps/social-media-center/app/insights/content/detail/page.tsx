...
type DetailData = {
  ...
  snapshot: { snapshot_time: string; play_count: number | null; like_count: number | null; comment_overview_count: number | null; favorite_count: number | null; share_count: number | null; post_age_days: number; actual_loaded_count: number | null; comment_rows_count: number; source_record_status: string; source_failure_reason: string | null } | null;
  ...
  snapshotHistory: { id: number; snapshot_date: string | null; snapshot_time: string; play_count: number | null; like_count: number | null; comment_overview_count: number | null; favorite_count: number | null; share_count: number | null; source_record_status: string }[];
  evaluationHistory: { evaluation_date: string; snapshot_id: number; total_score: number | null; grade: string | null; propagation_score: number | null; interaction_score: number | null; attraction_score: number | null; efficiency_score: number | null; confidence: string; douyin_paid_status: string; data_completeness: number | null }[];
  deltas: { play: number | null; like: number | null; comment: number | null; favorite: number | null; share: number | null };
...
function DeltaValue({ value }: { value: number | null }) {
  if (value === null) return <small className="snapshot-delta unavailable">暂无上次快照</small>;
  return <small className={`snapshot-delta ${value < 0 ? "negative" : value > 0 ? "positive" : ""}`}>较上次 {value > 0 ? "+" : ""}{formatCompact(value)}</small>;
}
...
export default function ContentDetailPage({ embedded = false }: ContentDetailPageProps = {}) {
  ...
  const snapshotSeries = (key: "play_count" | "like_count" | "comment_overview_count" | "favorite_count" | "share_count") => data.snapshotHistory
    .filter((item) => item.source_record_status !== "private" && typeof item[key] === "number")
  ...
  return <div className={`page-stack content-detail-page platform-themed-page theme-${post.platform} ${embedded ? "v2-embedded-business-page" : ""}`}>
    ...
    {activeTab === "traffic" && <section className="detail-analysis-grid traffic-overview-grid">
      <div className="detail-column">
        <article className="panel detail-panel"><div className="detail-panel-heading"><div><span className="section-kicker">BASIC PERFORMANCE</span><h2>基础表现</h2></div><span className="source-chip">social_post_snapshots</span></div>{data.snapshot?.source_record_status === "private" ? <AvailabilityMessage text="该作品为私密作品，平台未提供表现数据" /> : <div className="detail-count-grid"><div><span>播放</span><CountValue value={post.views} /><DeltaValue value={data.deltas.play} /></div><div><span>点赞</span><CountValue value={post.likes} /><DeltaValue value={data.deltas.like} /></div><div><span>平台评论总览</span><CountValue value={metrics.commentOverviewCount} /><DeltaValue value={data.deltas.comment} /></div><div><span>页面实际加载</span><CountValue value={metrics.actualLoadedCount} /></div><div><span>JSON评论记录</span><CountValue value={metrics.collectedCommentCount} /></div><div><span>收藏</span><CountValue value={post.favorites} /><DeltaValue value={data.deltas.favorite} /></div><div><span>分享</span><CountValue value={post.shares} /><DeltaValue value={data.deltas.share} /></div></div>}{post.hasPaidTraffic && <div className="paid-traffic-summary"><strong>DOU+ 付费播放 {formatCompact(post.paidViews)}</strong>{post.paidTrafficRelationship === "additional" ? <span>基础播放 {formatCompact(post.views ?? 0)} · DOU+为平台标注的额外流量 · 合计曝光 {formatCompact(post.totalExposure ?? post.views ?? 0)}</span> : <span>基础播放 {formatCompact(post.views ?? 0)} · 自然播放口径 {post.organicViews === null ? "平台未说明" : formatCompact(post.organicViews)}</span>}<p>DOU+ 独立保存，AI 爆款判断不把付费播放直接计为自然传播能力。</p></div>}</article>
    ...
    {activeTab === "trend" && <div className="detail-trend-stack"><section className="panel detail-panel"><div className="detail-panel-heading"><div><span className="section-kicker">SYSTEM DAILY SNAPSHOTS</span><h2>系统每日快照趋势</h2></div><span className="source-chip">social_post_snapshots</span></div><div className="audience-dimension-grid"><SeriesChart title="播放累计变化" points={snapshotSeries("play_count")} /><SeriesChart title="点赞累计变化" points={snapshotSeries("like_count")} color="#30bde0" /><SeriesChart title="评论累计变化" points={snapshotSeries("comment_overview_count")} color="#eb6b4a" /><SeriesChart title="收藏累计变化" points={snapshotSeries("favorite_count")} color="#c65454" /><SeriesChart title="分享累计变化" points={snapshotSeries("share_count")} color="#8d68c8" /></div><p className="data-quality-notice">按每次 WorkBuddy 日监测快照形成生命周期曲线；下降值按平台真实返回保留，不标记为系统异常。</p></section><section className="panel detail-panel"><div className="detail-panel-heading"><div><span className="section-kicker">PLATFORM METRIC SERIES</span><h2>抖音平台趋势</h2></div><span className="source-chip">social_post_metric_series</span></div><div className="audience-dimension-grid"><SeriesChart title="播放趋势" points={data.metricSeries.filter((point) => point.metric_type === "play" && point.series_name === "hourly_new")} /><SeriesChart title="作品吸粉趋势" points={data.metricSeries.filter((point) => point.metric_type === "follower_gain" && ["hourly_new", "daily_new"].includes(point.series_name))} color="#30bde0" /><SeriesChart title="内容留存 · 当前作品" points={data.metricSeries.filter((point) => point.metric_type === "retention" && point.series_name === "current")} color="#eb6b4a" /><SeriesChart title="跳出趋势 · 当前作品" points={data.metricSeries.filter((point) => point.metric_type === "bounce" && point.series_name === "current")} color="#c65454" /></div><p className="data-quality-notice">仅展示 WorkBuddy 从抖音页面图表实例真实读取的数据点，与系统每日快照趋势分开呈现。</p></section></div>}
