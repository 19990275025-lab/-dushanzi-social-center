...
type DetailData = {
  post: { id: number; platform: string; title: string; content_type: string; publish_time: string; cover_url: string | null; views: number; likes: number; comments: number; favorites: number; shares: number; fans_growth: number; interactions: number; duration: number | null };
  ...
  return (
    <div className={`page-stack content-detail-page platform-themed-page theme-${post.platform}`}>
      ...
      {activeTab === "traffic" && <section className="detail-analysis-grid">
        ...
        <div className="detail-column">
          <article className="panel detail-panel traffic-source-panel"><div className="detail-panel-heading"><div><span className="section-kicker">TRAFFIC SOURCE</span><h2>流量来源</h2></div><span className="pending-chip">待采集</span></div>{data.trafficSources.length ? <div className="traffic-source-list">{data.trafficSources.map((item) => <div key={item.name}><span>{item.name}</span><i><b style={{ width: `${item.rate}%` }} /></i><strong>{item.rate}%</strong></div>)}</div> : <div className="detail-empty-state"><strong>暂无平台流量来源明细</strong><p>后续接入抖音创作者后台指标后，将显示推荐页、个人主页、关注页等来源占比。</p></div>}</article>
      ...
      {activeTab === "audience" && <section className="detail-analysis-grid">
        ...
        <article className="panel detail-panel"><div className="detail-panel-heading"><div><span className="section-kicker">AUDIENCE PROFILE</span><h2>观众画像</h2></div><span className="pending-chip">待采集</span></div><div className="detail-empty-state tall"><strong>暂无该作品观众画像</strong><p>{data.dataAvailability.note}</p><a href="/insights/fans">查看账号粉丝分析 →</a></div></article>
      </section>}
