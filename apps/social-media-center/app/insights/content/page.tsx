  ...
  return (
    <div className={`page-stack content-insights-page platform-themed-page theme-${platform}`}>
      ...
      <section className="panel breakout-panel">
        ...
        <div className="table-wrap"><table className="content-table"><thead><tr><th>排名</th><th>作品</th><th>平台</th><th>播放</th><th>互动</th><th>涨粉</th><th>发布时间</th><th>操作</th></tr></thead><tbody>{data.topPosts.map((post, index) => <tr key={post.id}><td><span className={`rank-chip rank-${index + 1}`}>TOP {index + 1}</span></td><td><strong>{post.title}</strong><small className="table-subline">{contentTypeLabels[post.content_type] ?? post.content_type}</small></td><td><span className={`platform-tag tag-${post.platform}`}>{platformLabel(post.platform)}</span></td><td className="metric-cell">{formatCompact(post.views)}</td><td>{formatCompact(post.interactions)}</td><td>{formatCompact(post.fans_growth)}</td><td className="date-cell">{formatDate(post.publish_time)}</td><td><a className="content-analysis-link" href={`/insights/content/detail?id=${post.id}`}>数据分析</a></td></tr>)}{!data.topPosts.length && <tr><td className="empty-cell" colSpan={8}>当前平台暂无作品数据</td></tr>}</tbody></table></div>
      </section>
