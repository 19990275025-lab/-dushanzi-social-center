...
function aggregatePlatforms(items: FanPlatform[]): FanPlatform {
  ...
  for (const item of items) {
    for (const point of item.trend) {
      ...
      current.fans_count += point.fans_count;
      current.net_growth += point.net_growth;
      current.new_fans += point.new_fans;
      current.lost_fans += point.lost_fans;
      trendMap.set(point.record_date, current);
    }
  }
  const trend = [...trendMap.values()].sort((left, right) => left.record_date.localeCompare(right.record_date));
  return {
    platform: "all",
    fansCount: items.reduce((sum, item) => sum + item.fansCount, 0),
    netGrowth: items.reduce((sum, item) => sum + item.netGrowth, 0),
    trend,
    trendSource: "四平台增长记录汇总",
    profile: null,
  };
}
...
export default function FanInsightsPage() {
  ...
  const pageTitle = selected === "all" ? "粉丝分析" : `${currentPlatformLabel}粉丝分析`;
  ...
  return (
    <div className={`page-stack fan-insights-page platform-themed-page theme-${selected}`}>
      <header className="page-heading compact-heading">
        <div><p className="eyebrow">FAN ANALYSIS</p><h1>{pageTitle}</h1><p>本页只展示粉丝规模、增长和画像，不混入作品排行与内容建议。</p></div>
      ...
      <section className="fan-platform-grid">
        {platformData.map((item) => <button aria-pressed={selected === item.platform} className={`fan-platform-card platform-${item.platform} ${selected === item.platform ? "active" : ""}`} key={item.platform} onClick={() => setSelected(item.platform)}><div><span>{item.platform === "all" ? "全部平台" : platformLabel(item.platform)}</span><small>{selected === item.platform ? "当前平台" : item.platform === "all" ? "四平台汇总" : item.profile ? "画像已采集" : "画像待采集"}</small></div><strong>{formatCompact(item.fansCount)}<em>粉丝</em></strong><p className={item.netGrowth >= 0 ? "growth-up" : "growth-down"}>{item.netGrowth >= 0 ? "+" : ""}{formatCompact(item.netGrowth)} 期间增长</p></button>)}
      </section>

      <nav className="insight-platform-tabs fan-tabs" aria-label="粉丝分析平台筛选">
        {platforms.map((item) => <button aria-pressed={selected === item} className={`${selected === item ? "active" : ""} platform-tab-${item}`} key={item} onClick={() => setSelected(item)}>{item === "all" ? "全部平台" : platformLabel(item)}</button>)}
      </nav>
      ...
      <section>
        ...
        <div className="fan-profile-grid">
          ...
          <ProfileBlock title="兴趣" items={current.profile?.interests ?? []} emptyMessage={selected === "all" ? "请选择具体平台查看画像" : undefined} />
