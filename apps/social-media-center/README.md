...
页面顶部按“全部热点、抖音热点、快手热点、微博热点”分类，`GET /api/hot-topics?platform=` 在服务端完成平台过滤。V2.1 进一步拆分“平台热点趋势”和“内容热度分析”：前者只读取平台榜单，后者展示热门视频、搜索热词和爆款内容。`hot_topics.data_source` 支持 `douyin_hot_rank`、`douyin_seed_rank`、`douyin_challenge_rank`、`douyin_content_hot`，并建立 `(platform, data_source, ranking)` 索引。
