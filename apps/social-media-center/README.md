...
- `/insights/content`：独立“内容监测中心”，包含四平台作品监测、五类内容分析、同行对比、AI 日报和原作品详情能力。
...
`/insights/fans` 已拆分为左侧一级模块“03 粉丝分析中心”。页面继续读取 `social_accounts`、`social_fans` 和 `fan_growth_records`，提供粉丝数量、新增粉丝、增长率、7天/30天/自然月趋势及地域、年龄、性别、兴趣、活跃时间画像，并按抖音流量获取、快手用户关系、微博品牌传播、视频号私域沉淀生成差异化建议和 AI 粉丝运营周报。

原 `/insights` 分流页、内容转粉、作品详情和自动采集兼容说明全部保留。真实画像尚未采集时保持空状态；增长序列缺失时继续将 `social_posts.fans_growth` 作为明确标注的回退趋势。

### 平台主题色切换 V1.0

内容监测中心和粉丝分析中心均支持“全部平台、抖音、快手、微博、视频号”五种状态。全部平台沿用营销中台绿色；抖音使用纯黑主色并搭配红色、青色强调，快手使用纯橙色，微博使用纯红色，视频号使用纯微信绿并保留橙色强调。平台主题全部使用纯色，不使用渐变；主题只作用于平台 Tab、洞察卡片顶部、图表和关键指标，不改变页面背景或其他模块。
