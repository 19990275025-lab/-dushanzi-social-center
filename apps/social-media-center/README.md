...
- `/insights/content`：独立“内容监测中心”，包含抖音、快手、微博作品监测、五类内容分析、同行对比、AI 日报和原作品详情能力。
...
- `GET /api/insights/content`：读取 `social_posts`、`hot_topics` 和 `competitor_posts`，生成作品评分、五类内容占比、同行对比、AI 日报、爆款和内容转粉关联。
...
新增 `competitor_posts` 保存同行作品指标，首批重点对比对象包括那拉提景区、喀纳斯景区、天山天池和赛里木湖，同时允许接入风景、亲子、旅游、游玩类账号。没有真实竞品作品时明确显示“待采集”，不生成模拟指标。表中预留 `source_type`、`source_record_id`、`raw_payload` 和 `collection_log_id`，便于未来 Chrome、Excel 或平台 API 采集。
