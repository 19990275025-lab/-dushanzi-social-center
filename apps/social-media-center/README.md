...
一键生成任务会写入既有 `content_tasks`，任务来源和热点 ID 保存在审核备注，并通过 `content_plans.task_id` 保持关联。任务发布后可选择已有抖音 `social_posts`，系统同步展示播放、赞评藏转；作品发布时间满7天后，定时任务会生成 `content_plan_feedback`，输出达成情况、优势、不足和是否继续推荐类似内容。V1.0 不开放快手、微博策划入口，也不生成模拟作品指标。
