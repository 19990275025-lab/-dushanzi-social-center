...
- `/collector`：统一管理 Chrome 自动采集、Excel 人工导入、数据模板、校验和采集日志；V1.0 仅开放抖音自动采集。
...
智能采集中心将五项能力汇总到 `/collector`：Chrome 自动采集、Excel 人工导入、数据模板管理、统一数据校验和采集日志。两种采集方式最终均写入 `social_posts`：Excel 数据通过 `import_log_id` 追踪，Chrome 数据通过 `collection_log_id` 追踪。
...
扩展不读取 Cookie、密码或浏览历史，不绕过登录、验证码、风控或平台限制，也不直接连接数据库。抖音页面结构变化后，需升级扩展中的页面适配器。快手、微博和视频号的自动采集仍为后续阶段，当前继续使用 Excel 模板与人工导入。
