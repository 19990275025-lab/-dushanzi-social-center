...
export const socialPosts = sqliteTable(
  ...
  (table) => [
    uniqueIndex("uq_social_posts_account_title").on(table.accountId, table.title).where(sql`${table.platform} <> 'kuaishou' OR ${table.platformPostId} IS NULL`),
    uniqueIndex("uq_social_posts_platform_post_id").on(table.platform, table.accountId, table.platformPostId),
...
export const socialPostEvaluations = sqliteTable(
  "social_post_evaluations",
  {
    ...
    naturalPerformanceConfidence: text("natural_performance_confidence"),
    viewingScore: real("viewing_score"),
    followerScore: real("follower_score"),
...
export const socialPostPaidTraffic = sqliteTable(
  "social_post_paid_traffic",
  {
    ...
    promotionType: text("promotion_type").notNull().default("unknown"),
    promotionSource: text("promotion_source"),
    promotionPresent: integer("promotion_present", { mode: "boolean" }),
    playCount: integer("play_count"),
...
export const socialPostTrafficSources = sqliteTable(
  "social_post_traffic_sources",
  {
    ...
    snapshotTime: text("snapshot_time").notNull(),
    sourceType: text("source_type").notNull(),
    metricDimension: text("metric_dimension").notNull().default("unknown"),
    sourceName: text("source_name").notNull(),
  ...
  (table) => [
    uniqueIndex("uq_social_post_traffic_source_snapshot").on(table.snapshotId, table.metricDimension, table.sourceName, table.trafficNature),
...
export const socialComments = sqliteTable(
  "social_comments",
  {
    ...
    fieldAvailability: text("field_availability", { mode: "json" }).$type<Record<string, string> | null>(),
