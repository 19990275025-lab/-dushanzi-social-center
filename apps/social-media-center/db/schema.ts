...
export const hotTopics = sqliteTable(
  ...
  (table) => [
    uniqueIndex("uq_hot_topics_platform_source_name").on(table.platform, table.dataSource, table.topicName),
    uniqueIndex("uq_hot_topics_non_douyin_name").on(table.platform, table.topicName).where(sql`platform <> 'douyin'`),
